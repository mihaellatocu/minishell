...
/*manipulate the string from token list, removing the $env/word and replace it with
	value of the matching key*/
static char	*create_new_word(char *current_word, char *env, char *to_replace, int start)
{
	int		i;
	int		j;
	char	*new_word;
...
static char	*replace_dollar_sign(char *word, t_env_list *env_list, int status)
{
	...
	to_replace = NULL;
	if (strncmp(word, "$?", ft_strlen(word)) == 0) //why not 3 or strlen + 1?
	{
...
int	ft_find_dollar(char *str, char **word, int i)
{
	...
	while (str[++i] != '\0')
	{
		if (str[i] == '$' && str[i + 1] != '\0' && (str[i + 1] == '_' || ft_isalpha(str[i + 1]) \
		|| str[i + 1] == '?'))
		{
			start = i + 1;
			break ;
	...
	while (current)
	{
		...
		if (current->token == WORD || current->token == DQUOTE)
		{
			...
			while (count > 0)
			{
				...
				if (i >= 0)
					current->content = create_new_word(current->content, word, replace_dollar_sign(word, p->envir, p->command_status), i);
				if (word)
					free(word);
				count--;
			}
